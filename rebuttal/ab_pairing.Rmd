...
#dt.ann.ab.s2 = dt.ann.a.s %>%
#  select(pa, antigen.epitope) %>%
#  merge(dt.ann.b.s %>%
#          select(pb, antigen.epitope)) %>%
#  summarise(not.found = sum(pa == 0 & pb == 0),
#            found.both = sum(pa > 0 & pb > 0),
#            found.aonly = sum(pa > 0 & pb == 0),
#            found.bonly = sum(pa == 0 & pb > 0))
#
#
#tmp = with(dt.ann.ab.s2, matrix(c(not.found, found.bonly, found.aonly, found.both), 
#           nrow = 2, ncol = 2))
#tt = tableGrob(tmp, 
#           rows = c("A-", "A+"), cols = c("B-", "B+"),
