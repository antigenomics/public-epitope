...
dt.hip.stats = fread("annotations/hip_stats.txt") %>%
  ...
  select(sample_id, race, count_total, occurrences_total)
```
...
```{r}
dt.hip.s = dt.hip.m %>%
  group_by(race, hla_spec, antigen.epitope, antigen.species) %>%
...
```{r}
dt.hip.s.s = dt.hip.s %>%
  filter(race == "caucasian,non-hispanic or latino") %>%
  group_by(antigen.epitope) %>%
...
ggplot(dt.hip.s, aes(x = antigen.epitope, fill = race, y = occurrences / occurrences_total)) +
...
ggplot(dt.hip.s, aes(x = antigen.species, fill = race, y = occurrences / occurrences_total)) +
  geom_boxplot(aes(group = paste(antigen.species, race))) +
  coord_flip() +
  scale_fill_brewer(palette = "Set1") +
  xlab("") + ylab("Fraction of rearrangements") +
  theme_bw()

ggplot(dt.hip.s, aes(x = hla_spec, fill = race, y = occurrences / occurrences_total)) +
  geom_boxplot(aes(group = paste(hla_spec, race))) +
  coord_flip() +
  scale_fill_brewer(palette = "Set1") +
  xlab("") + ylab("Fraction of rearrangements") +
  theme_bw()
